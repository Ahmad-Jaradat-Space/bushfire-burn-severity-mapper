...
def test_unet_forward_18ch_cpu():
    ...
    m = build_unet(in_channels=18, num_classes=4, encoder_name="resnet34", encoder_weights=None)
    m.eval()
...
def test_unet_smaller_encoder():
    ...
    m = build_unet(in_channels=18, num_classes=4, encoder_name="mobilenet_v2", encoder_weights=None)
    m.eval()
...
def test_combo_loss_with_ignore():
    ...
    loss, comps = combo_loss(logits, target, ce_weight=0.5, dice_weight=0.5, ignore_index=255)
