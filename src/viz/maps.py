...
def render_severity_map(
    arr: np.ndarray, out_path: Path, title: str = "", figsize: tuple[int, int] = (8, 6)
) -> Path:
    apply_theme()
...
def render_aoi_locator(
    aois: list[tuple], out_path: Path, figsize: tuple[float, float] = (10.5, 8.0)
) -> Path:
    ...
    ax.add_feature(cfeature.LAND.with_scale("50m"), facecolor="#EFEAD9", edgecolor="none")
    ax.add_feature(cfeature.OCEAN.with_scale("50m"), facecolor="#DCE6EA", edgecolor="none")
    ax.add_feature(cfeature.COASTLINE.with_scale("50m"), edgecolor="#5C5147", linewidth=0.7)
    ax.add_feature(cfeature.STATES.with_scale("50m"), edgecolor="#B0A48C", linewidth=0.5, alpha=0.7)
    ...
    for event_id, label, state, split, colour in aois:
        ...
        ax.annotate(
            ...
            bbox=dict(boxstyle="round,pad=0.4", fc="white", ec=colour, alpha=0.95, lw=0.7),
            zorder=6,
        )
        ax.scatter(
            [cx], [cy], s=55, c=colour, edgecolor="white", linewidth=1.4, zorder=7, transform=proj
        )
    ...
    ax.set_title(
        "Four areas of interest, spanning three states and three ecological zones",
        loc="left",
        fontsize=13,
        pad=14,
        color=INK,
    ...
    for _, _, _, split, c in aois:
        if split not in seen:
            handles.append((split, c))
            seen.add(split)
    chips = [
        plt.Line2D(
            [0], [0], marker="s", color=c, markersize=11, linestyle="None", label=split.upper()
        )
