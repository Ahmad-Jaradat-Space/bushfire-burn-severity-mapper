...
from src.viz.synthetic_scene import synthetic_model_predictions
...
class KangarooScene:
    ...
    n_pre_scenes: int | None = None
    n_post_scenes: int | None = None
    clear_fraction_pre: float | None = None
    clear_fraction_post: float | None = None
...
def load_kangaroo(
    force_synthetic: bool = False, event_id: str = "kangaroo_island_2019_2020"
) -> KangarooScene:
    interim = REPO_ROOT / "data" / "interim" / event_id
    # Mask filename convention varies between pipeline versions: accept either.
    pre_mask = next(
        (interim / n for n in ("mask_pre_10m.tif", "pre_mask_10m.tif") if (interim / n).exists()),
        interim / "mask_pre_10m.tif",
    )
    post_mask = next(
        (interim / n for n in ("mask_post_10m.tif", "post_mask_10m.tif") if (interim / n).exists()),
        interim / "mask_post_10m.tif",
    )
    paths = {
    ...
    if not have_real:
        ...
        return KangarooScene(
            ...
            burn_mask=s.burn_mask,
            is_real=False,
            source_label="synthetic stand-in (deterministic seed=7)",
        )
    ...
    try:
        ...
        for kind, attr_n, attr_c in [
            ("pre_stack_10m.tif", "n_pre", "clear_pre"),
    ...
    return KangarooScene(
        ...
        clear_fraction_pre=clear_pre,
        clear_fraction_post=clear_post,
    )


def model_predictions(
    scene: KangarooScene, seed: int = 0
