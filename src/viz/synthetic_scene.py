...
def s2_swir_falsecolour(stack: np.ndarray, gamma: float = 0.85, scale: float = 2.6) -> np.ndarray:
...
def _noise_field(h: int, w: int, scale: float, seed: int) -> np.ndarray:
    ...
    y1 = np.minimum(y0 + 1, lo_h - 1)
    x0 = np.floor(xs).astype(int)
    x1 = np.minimum(x0 + 1, lo_w - 1)
    fy = (ys - y0)[:, None]
    fx = (xs - x0)[None, :]
    a = lo[np.ix_(y0, x0)]
    b = lo[np.ix_(y0, x1)]
    c = lo[np.ix_(y1, x0)]
    d = lo[np.ix_(y1, x1)]
    return (a * (1 - fx) * (1 - fy) + b * fx * (1 - fy) + c * (1 - fx) * fy + d * fx * fy).astype(
        np.float32
    )
...
def build(h: int = 512, w: int = 900, seed: int = 7) -> SyntheticScene:
    ...
    for cx_p, cy_p, r_p in [
        (cx + rx * 0.30, cy - ry * 0.20, 12.0),
        (cx + rx * 0.42, cy + ry * 0.10, 9.0),
    ]:
        m = ((xx - cx_p) ** 2 + (yy - cy_p) ** 2) < r_p**2
        m = m & land
        for c, v in [(0, 0.045), (1, 0.060), (2, 0.045), (3, 0.025), (4, 0.015), (5, 0.010)]:
...
def synthetic_model_predictions(severity: np.ndarray, seed: int = 0) -> dict[str, np.ndarray]:
