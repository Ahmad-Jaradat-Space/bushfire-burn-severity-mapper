...
def predict_tree_model(model_path: Path, out_dir: Path, model_name: str) -> None:
    ...
    for ev in EVENTS:
        ...
        write_manifest(
            out,
            event_id=ev,
            pipeline_step=f"{model_name}.predict",
            inputs={"model_path": str(model_path.relative_to(REPO_ROOT))},
            crs=str(crs),
            resampling=None,
        )
        hist = dict(zip(*np.unique(pred, return_counts=True), strict=False))
        log.info(
            "[%s/%s] wrote %s   class hist=%s", model_name, ev, out.relative_to(REPO_ROOT), hist
        )


def predict_segmentation_model(
    model_kind: str, config_path: str, weights_path: Path, stats_path: Path, out_dir: Path
) -> None:
    ...
    model.eval()
    log.info(
        "[%s] loaded weights from %s on %s",
        model_kind,
        weights_path.relative_to(REPO_ROOT),
        device_info.name,
    )
    ...
    for ev in EVENTS:
        ...
        pred = _sliding_window_predict(
            model, image, mean, std, tile=tile, stride=stride, device=device, num_classes=4
        )
        pred[~mask] = 255
        _write_uint8(out, pred, transform, crs)
        write_manifest(
            out,
            event_id=ev,
            pipeline_step=f"{model_kind}.predict",
        ...
        hist = dict(zip(*np.unique(pred, return_counts=True), strict=False))
        log.info(
            "[%s/%s] wrote %s   class hist=%s", model_kind, ev, out.relative_to(REPO_ROOT), hist
        )
...
def predict_dnbr_baseline(out_root: Path) -> None:
    for ev in EVENTS:
        ...
        write_manifest(
            out,
            event_id=ev,
            pipeline_step="baseline_dnbr.predict",
