...
from src.models.baselines import dnbr as dnbr_index
from src.models.baselines import dnbr_multiclass_usgs
from src.viz.maps import render_aoi_locator
from src.viz.scene_loader import load_kangaroo, model_predictions
from src.viz.synthetic_scene import s2_swir_falsecolour, s2_truecolour
...
def fig02_prepost_truecolour():
    ...
    src = (
        "Real Sentinel-2 L2A via Microsoft Planetary Computer"
        if scene.is_real
...
def fig04_five_methods():
    ...
    def _t(k, base):
        if k not in preds:
            return f"{base} · not trained"
        if not preds_real.get(k, False):
            return f"{base} · synthetic stand-in"
        return base
    ...
    panels = [
        ...
        (
            ...
            preds.get("baseline_dnbr"),
            "baseline_dnbr" in preds,
        ),
        (_t("rf", "RandomForest"), preds.get("rf"), "rf" in preds),
        (_t("xgb", "XGBoost"), preds.get("xgb"), "xgb" in preds),
        (_t("unet", "U-Net (ResNet-34)"), preds.get("unet"), "unet" in preds),
        (_t("segformer", "SegFormer-B0"), preds.get("segformer"), "segformer" in preds),
    ]
    for ax, (title, arr, present) in zip(axes.ravel(), panels, strict=False):
        if arr is not None and present:
            ax.imshow(
                np.ma.masked_equal(arr, 255),
                cmap=severity_cmap(),
                vmin=0,
                vmax=3,
                interpolation="nearest",
...
def fig05_eventwise_iou():
    ...
    b1 = ax.bar(
        ...
        iou_random,
        width=w,
        label="Random tile split (smoke test, Kangaroo only)",
        color=ACCENT_BLUE,
        alpha=0.65,
    )
    b2 = ax.bar(
        ...
        iou_eventwise,
        width=w,
        label="Event-wise hold-out (Currowan+Gospers → Kangaroo)",
        color=ACCENT,
        alpha=0.95,
    )
    for b, v in zip(b1, iou_random, strict=False):
        ax.text(
    ...
    for b, v in zip(b2, iou_eventwise, strict=False):
        ax.text(
            b.get_x() + b.get_width() / 2, v + 0.012, f"{v:.2f}", ha="center", fontsize=9, color=INK
        )
    for x, vr, ve in zip(xs, iou_random, iou_eventwise, strict=False):
    ...
    ax.set_ylim(0, 0.32)
    ax.set_ylabel("Macro IoU vs GEEBAM proxy (4 classes)")
    ax.set_title(
        "What spatial leakage looks like when you remove it", loc="left", fontsize=13, pad=14
    )
