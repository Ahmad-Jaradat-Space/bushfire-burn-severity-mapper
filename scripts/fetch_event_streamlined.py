...
def main():
    ...
    _flush_print(
        f"AOI bbox (WGS84): {bbox_wgs}; working CRS EPSG:{epsg}; " f"resolution={args.resolution}m"
    )
    ...
    if args.side in ("pre", "both"):
        ...
        _flush_print(
            f"selected {len(pre_items)} pre items, clouds: "
            + ", ".join(f"{it['properties'].get('eo:cloud_cover', 0):.1f}%" for it in pre_items)
        )
        pre_signed = _sign_items(pre_items)
        pre_meta = composite_one_side(
            pre_signed,
            args.resolution,
            epsg,
            interim_dir / "pre_stack_10m.tif",
            n_scenes=len(pre_signed),
            bbox_wgs=bbox_wgs,
        )
        write_manifest(
            interim_dir / "pre_stack_10m.tif",
            event_id=event_id,
            pipeline_step="streamlined_fetch.pre",
            inputs={
                "stac_items": [it["id"] for it in pre_items],
                "n_scenes": pre_meta["n_scenes"],
                "bands": list(REFLECT),
                "resolution_m": args.resolution,
                "composite": "nanmedian",
                "clear_fraction": pre_meta["clear_fraction"],
    ...
    if args.side in ("post", "both"):
        ...
        _flush_print(
            f"selected {len(post_items)} post items, clouds: "
            + ", ".join(f"{it['properties'].get('eo:cloud_cover', 0):.1f}%" for it in post_items)
        )
        post_signed = _sign_items(post_items)
        post_meta = composite_one_side(
            post_signed,
            args.resolution,
            epsg,
            interim_dir / "post_stack_10m.tif",
            n_scenes=len(post_signed),
            bbox_wgs=bbox_wgs,
        )
        write_manifest(
            interim_dir / "post_stack_10m.tif",
            event_id=event_id,
            pipeline_step="streamlined_fetch.post",
            inputs={
                "stac_items": [it["id"] for it in post_items],
                "n_scenes": post_meta["n_scenes"],
                "bands": list(REFLECT),
                "resolution_m": args.resolution,
                "composite": "nanmedian",
                "clear_fraction": post_meta["clear_fraction"],
